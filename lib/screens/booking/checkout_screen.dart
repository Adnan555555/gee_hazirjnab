...
class _CheckoutScreenState extends State<CheckoutScreen> {
  // Phone controller removed - using profile mobile
  ...
  bool _isImmediately = false; // Default: Schedule (not immediately)
  ...
  void initState() {
    super.initState();
    // Initialize address from provider
          ...
          return SingleChildScrollView(
            child: Column(
              ...
              children: [
                  ...
                  child: Container(
                    ...
                    child: Column(
                      ...
                      children: [
                        ...
                        // Immediately badge when selected
                        if (_isImmediately)
                          Padding(
                ...
                // Date/Time Section (only show when NOT immediately)
                if (!_isImmediately) ...[
                  // Schedule Section
                ...
                Consumer<AuthProvider>(
                  builder: (context, authProvider, child) {
                    ...
                    // Use selected address or fallback to default/first address
                    ...
                    child: Row(
                      children: [
                        Container(
                          ...
                          child: const Icon(Icons.build, color: AppTheme.textSecondary),
                        ),
                        const SizedBox(width: 12),
                        Expanded(
                          child: Column(
                          ...
                          child: Row(
                            children: [
                              ...
                              InkWell(
                                onTap: () {
                                  // Increase quantity
                                  cartProvider.incrementCartItem(item.serviceId);
                                },
                                child: const Padding(
                                  padding: EdgeInsets.all(8),
                      ...
                      ...cartItems.map((item) => Padding(
                        ...
                        child: Row(
                          ...
                          children: [
                            ...
                            Text('${item.salePrice.toStringAsFixed(0)} x ${item.quantity}'),
                          ],
                        ),
                      )),
                      const Divider(),
  ...
  Future<void> _placeOrder() async {
    ...
    // Get mobile number from auth provider
    final authProvider = context.read<AuthProvider>();
    final cartProvider = context.read<CartProvider>();
    final mobile = authProvider.customer?.mobile;
    debugPrint('Placing order for mobile: $mobile');
    
    // Use selected address or fallback to provider address
    final finalAddress = _selectedAddress ?? 
        authProvider.customer?.defaultAddress ?? 
        (authProvider.customer?.addresses.isNotEmpty == true ? authProvider.customer!.addresses.first : null);
    ...
    try {
      ...
      if (_problemImage != null) {
        debugPrint('Uploading problem image: ${_problemImage!.path}');
                  ...
                  return ListView.separated(
                    ...
                    itemBuilder: (context, index) {
                      ...
                      if (_selectedAddress?.id != null && address.id != null) {
                        ...
                        // Fallback: compare content if ID is null
              ...
              onPressed: () async {
                ...
                if (result == true) {
                  ...
                  if (mounted) {
                    ...
                    // Select the last added address (assuming it's at the end or default)
